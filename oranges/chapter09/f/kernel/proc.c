...
PUBLIC int sys_sendrec(int function, int src_dest, MESSAGE* msg, struct proc* p)
{
	...
	if((src_dest >= 0 && src_dest < NR_PROCS + NR_TASKS) ||
...
PRIVATE int msg_send(struct proc* current, int dest, MESSAGE* msg)
{
			...
			p_dest->p_recvfrom == ANY))
	{
		...
		// printf("function:send,dest:%d, sender:%d\n", dest, proc2pid(sender));
...
PRIVATE int msg_receive(struct proc* current, int src, MESSAGE* msg)
{
	...
	if(copyok)
	{
		...
		assert(p_from->p_msg);
		/* copy the message */
		// printf("function:receive,dest:%d, sender:%d\n", proc2pid(p_who_wanna_recv), src);
