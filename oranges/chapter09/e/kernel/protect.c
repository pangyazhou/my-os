...
PUBLIC void init_prot()
{
    ...
    /* 填充 GDT 中 TSS 这个描述符 */
    memset(&tss, 0, sizeof(tss)); 		/* 用0将tss结构填充 */
    tss.ss0 = SELECTOR_KERNEL_DS;
    init_descriptor(&gdt[INDEX_TSS],
    	vir2phys(seg2phys(SELECTOR_KERNEL_DS), &tss),
    	sizeof(tss) - 1,
    	DA_386TSS);
    tss.iobase = sizeof(tss); 	/* 没有io许可位 */
...
PRIVATE void init_idt_desc(unsigned char vector, u8 desc_type,
				int_handler handler, unsigned char privilege)
{
	GATE* p_gate = &idt[vector];	//要设置的中断门
	u32 base = (u32)handler; 		//中断处理函数偏移地址
...
PUBLIC void exception_handler(int vec_no, int err_code, int eip, int cs, int eflags)
{
	...
	char* err_msg[] = {
				...
				"#UD Invalid Opcode (Undefined Costruct procor)",
				"#NM Device Not Available (No Math Costruct procor)",
				"#DF Double Fault",
				"    Costruct procor Segment Overrun (reserved)",
				"#TS Invalid TSS",
				"#NP Segment Not Present",
				"#SS Stack-Segment Fault",
				"#GP General Protection",
				"#PF Page Fault",
				"--  (Inter reserved. Do not use.)",
